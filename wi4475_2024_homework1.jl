...
using Plots, DataFrames, CSV, HTTP, DifferentialEquations, StaticArrays
...
The idea for this question comes from [the Deep Learning book by Bishop](https://www.bishopbook.com/) (Chapter 2) where a curved coin is shown as an unfair probability between heads and tails. Based on this, I modified a few coins and made a 100 throws with each one.
![unfair coins](https://raw.githubusercontent.com/robot144/wi4475_2024/main/images/coins.png)

For each coin the thicknes (in mm) minus the thickness of a flat coin was measured. A sign was added to distinguish between the two bendig directionss. Positive values show tails in the figure. The assumption is that the rounded side on the table is somewhat prefered. Let's study that later.
"""
...
# ╔═╡ 18474b6e-8192-4413-9553-7f93556078a2
md"""
...
begin
   ...
   coins = read_remote_csv("https://raw.githubusercontent.com/robot144/wi4475_2024/main/coin.csv")
   #select where curvalue is 0.0, and assume that this is a fair coin
   fair=coins[coins.curvature .== 0.0, :] # flat coins where curvature is 0.0
   count_tails=sum(fair[:,:tails])
   N=length(fair[:,:tails]) 
   @show count_tails, N
end

# ╔═╡ c6bbf64a-b7e9-4487-85da-766366eaa77c
md"""
...
begin #sample to get you started.
	for i=1:3
		n = rand(1:5)
		@show n
	end
...
# ╔═╡ 199b7d40-b0d2-4d83-bd14-95065dd78f59
md"""
### 1e: Logistic model
...
begin
	σ(x)=1/(1+exp(-x))
	plot(σ,-5.0,5.0,label="σ(x)")
end

# ╔═╡ 877a40a4-032e-4dae-a795-4329e2b4d243
md"""
...
$θ_1^\prime = ω1$
$θ_2^\prime = ω2$
$ω_1^\prime = \frac{ -g(2m_1+m_2)sin(θ_1) -m_2 g sin(θ_1-2θ_2) - 2sin(θ_1-θ_2) m_2 (ω_2^2 l_2+ω_1^2 l_1 cos(θ_1-θ_2)) }
        { l_1(2m_1 + m_2 - m_2 cos(2θ_1 - 2θ_2)) }$
$ω_2^\prime = \frac{  2sin(θ_1-θ_2)(ω_1^2 l_1 (m_1+m_2) + g(m_1+m_2)cos(θ_1) +ω_2^2 m_2 l_2 cos(θ_1-θ_2)) }
        { l_2(2m_1 + m_2 - m_2 cos(2θ_1 - 2θ_2)) }$
...
# ╔═╡ 953d4d5b-f4c6-495a-9946-1b58cd4012a1
#Plot a double pendulum
function plot_double_pendulum(θ1, θ2, l1, l2; line_color=:red, line_width=3, marker_color=:blue, marker_size=10)
    x1 = l1 * sin(θ1)
    y1 = -l1 * cos(θ1)
    x2 = x1 + l2 * sin(θ2)
    y2 = y1 - l2 * cos(θ2)
    plot([0, x1, x2], [0, y1, y2], aspect_ratio=:equal, xlims=(-2.2, 2.2), ylims=(-2.2, 2.2), legend=false, lw=line_width, color=line_color, axes=false)
    scatter!([x1, x2], [y1, y2], color=marker_color, ms=marker_size)  
end

# ╔═╡ a0ba13fd-e596-4bc0-8151-54858f6c85b8
function animate_double_pendulum(sol, l1, l2, timestep=0.05, fps=30)
	# create an animation
    ...
    anim = @animate for i in 1:n
      u=sol(times[i])
      θ1, θ2, ω1, ω2 = u
      plot_double_pendulum(θ1, θ2, l1, l2)
    end
	return gif(anim, fps = fps)
end

# ╔═╡ c5c411b4-d889-481c-9f2d-9a05c8a06032
function double_pendulum(u, p, t)
    ...
    dθ1dt = ω1
    dθ2dt = ω2
    dω1dt = ( -g*(2*m1+m2)sin(θ1) -m2*g*sin(θ1-2*θ2) - 2*sin(θ1-θ2)*m2*(ω2^2*l2+ω1^2*l1*cos(θ1-θ2)) )/( l1*(2*m1 + m2 - m2*cos(2*θ1 - 2*θ2)) )
    dω2dt = (  2*sin(θ1-θ2)*(ω1^2*l1*(m1+m2) + g*(m1+m2)*cos(θ1) +ω2^2*m2*l2*cos(θ1-θ2)) )/( l2*(2*m1 + m2 - m2*cos(2*θ1 - 2*θ2)) )
    return  @SVector [dθ1dt, dθ2dt, dω1dt, dω2dt] # the @SVector creates a short-lived vector that's faster. It will work without, but a bit slower.
end

# ╔═╡ 974d2f4d-49d7-49a2-9875-ad3155141843
begin
   θ1= π; θ2=π/2; ω1=0.0; ω2=0.0
   u0 =  @SVector [θ1, θ2, ω1, ω2 ]
   m1 = 1.0; m2 = 1.0; l1 = 1.0; l2 = 1.0; g = 10.0
   p = [m1, m2, l1, l2, g]
   tspan = (0.0, 10.0)
   prob = ODEProblem(double_pendulum, u0, tspan, p)
   @time sol = solve(prob, Tsit5(), reltol=1e-6, abstol=1e-6)
   animate_double_pendulum(sol, l1, l2)
end

# ╔═╡ 79924228-b54d-494a-a694-22a95193b510
begin
	plot(sol,idxs=[1,2],labels=["θ1" "θ2"]) #plot θ1 and θ2
end
...
begin
	# compute f(x) for system dx/dt=f(x),
	f_x = double_pendulum([π/5, π/4, 1.0, 1.0],p,0.0)
	@show f_x
end
...
$E = E_{kinetic}+E_{potential} = \ldots$

"""

# ╔═╡ 7187dbd3-ec2c-407b-8037-15b0f2c9a6cd
function E(x,p)
	θ1, θ2, ω1, ω2 = x
    m1, m2, l1, l2, g = p
	# TODO
	return -l1*cos(θ1) # not correct, but just to get you started
end

# ╔═╡ 6c0aca27-0ef6-47b9-89ca-17d05437870e
begin 
	dt=0.1
...
manifest_format = "2.0"
project_hash = "73f60c17158e02d68fcc3f041a8ead7f28c61397"
