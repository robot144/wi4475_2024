...
using Plots, PlutoUI, Symbolics, ForwardDiff, DifferentialEquations
...
The Burgers' equation is non-linear. Before we can find its adjoint it must be linearized. For this, we use a constant equilibrium solution $u(x,t)=\overline{u}$, with $\overline{u}>0$.
To find the linearized equation, we'll substitute a perturbation 
$u(x,t)=\overline{u}+\epsilon u'(x,t)$ with [infinitesimal](https://en.wikipedia.org/wiki/Infinitesimal) scalar $\epsilon$. Find the equation for $u'(x,t)$, which will be the linearized equation.
"""
...
Find the adjoint equation by moving all terms for the linearized equations to one side, ie in the form $F(u')=0$. Next form the dot product 
$\int \int F(u'(x,t) \lambda(x,t) dx dt$ and reorder to 
$\int \int u'(x,t) F'(\lambda(x,t)) dx dt$, where $F'$ becomes the adjoint equation. 
...
# ╔═╡ 451f618e-464b-43b0-a3bf-d2f4d9ae859e
md"""
...
$\frac{d u_k}{dt} = -u_k (u_k-u_{k-1})/\Delta x + \nu (u_{k-1}-2u_k+u_{k+1})/{\Delta x}^2$
...
Write the equation in the form $d\mathbf{x}/dt=\mathbf{M}(\mathbf{x})$, with $\mathbf{x}$ as the state-vector $\mathbf{x} = [u_1, u_2, \ldots, u_K]'$. Then compute the Jacobian and the adjoint equation as $d \mathbf{\lambda}/dt = [\partial M/\partial \mathbf{x}]' \mathbf{\lambda}$
"""

# ╔═╡ 767ee2c5-09d4-4c6c-9197-36d4ef1076e3
md"""
Answer ...
"""

# ╔═╡ b41fbb2b-0351-4725-b0cb-fe739eb31902
md"""
### 1d: Interpretation of the adjoint state $\lambda$

Let's define a cost function that only directly depends on the state of the final time $T$, ie $J(\mathbf{x}(T))=1/2(\mathbf{z}(T)-\mathbf{H}\mathbf{x}(T))'(\mathbf{z}(T)-\mathbf{H}\mathbf{x}(T))$.
Show that the adjoint state $\mathbf{\lambda}(t)$ can be interpreted as: 
$\mathbf{\lambda}(t)=\frac{\partial J}{\partial \mathbf{x}(t)}$. 
"""

# ╔═╡ 3427cbb7-4f60-496a-a8e0-f6c9013ea735
md"""
Answer...
"""

# ╔═╡ 0c9139e9-551a-488b-b187-331b0192f499
md"""
### 1e: State augmentation with parameter $\nu$

State augmentation is a trick to artificially extend the variables in the state vector $\mathbf{x}(t)$ to cast a more complex problem into a known form. This is often easier than deriving the equations for the extended problem from scratch (which you should also be able to do). For example, one can extend the case above (1d) to include a parameter, by adding the parameter to the state vector, with the equation $dp/dt=0$. Use this approach to find the equations to find the gradient for the parameter $\nu$ in the semi-discretized Burgers' equation building on exercises 1c and 1d. As we see in 1d the adjoint at $t=0$ will give us $\mathbf{\lambda}(t)=\frac{\partial J}{\partial \mathbf{x}(t)}$. If we have included $\nu$ into this vector, this will include the derivative wrt $\nu$.
"""

# ╔═╡ fee7bafe-78e6-42cb-8286-7b181f87be49
md"""
Answer...
"""
...
The [logistic map](https://en.wikipedia.org/wiki/Logistic_map) is a simple yet interesting operator of the form:
...
# ╔═╡ acec2b0f-043a-4902-bb0b-ed189883da25
f(x,r)=r*x*(1.0-x); # define the logistic map

# ╔═╡ 8656c7a2-6872-4599-bca9-dffcdffd446c
@bind r Slider(0.0:0.1:4.0, default=1.8)
...
Why does this happen? And can you think of a sequence $y_{n+1}=g(y_n)$ where this doesn't happen? What are the requirements for such a function?
"""

# ╔═╡ 153a2a78-9db6-4525-9772-857b41e02803
begin
function f_n(f, x, r, n)
    return foldl((x, _) -> f(x,r), 1:n, init=x)
end
...
# ╔═╡ 24190c31-96c7-4044-84cf-1adbfd78d4c1
@bind n Slider(1:8, default=1)

# ╔═╡ 343be1fd-8d72-4842-bfab-0de17fc253d8
md"""
n=$(n)
"""

# ╔═╡ 7a7c2e51-a16b-47f5-8503-2f93a2e84fd7
md"""
x_$(n)=
"""

# ╔═╡ 7739a6d3-4a3d-4134-90bc-72f020dda4c5
f_n(f,x₀,rₛ,n)

# ╔═╡ 89432dd2-c068-498a-9bc5-cc63e148495e
md"""
$\frac{d x_n}{d x_0}=$
"""

# ╔═╡ a1ad566f-307d-464e-818e-bf327e10a690
Symbolics.derivative(f_n(f,x₀,rₛ,n),x₀)

# ╔═╡ 074cec9e-5047-4650-98c8-d53b05cb64f3
md"""
Answer ...
"""
...
So, instead of expressing the derivative directly, we aim for a sequence of steps that expresses the derivative. The first way to achieve this is to turn the function into a symbolic expression, compute the derivative symbolically, and finally generate code for this expression. So, here the pattern is from code to formula to code again. Below, you can see this in action. Assuming that an implementation of $df/dx$ exists, how can you compute $d x_n/dx_0$ in forward mode? With forward mode we mean first $d x_1/d x_0$ then $d x_2/d x_0$, etc.
"""

# ╔═╡ 692f4661-8700-4ad5-8e25-36a12b07fb18
begin
	# starting with f as a regular function.
	@variables xₛ
	f_symbolic = f(xₛ,rₛ) #evaluate the function with a symbolic variable as input
	@show f_symbolic #this gives a symbolic formula
	df_dx_symbolic=Symbolics.derivative(f_symbolic,xₛ) #compute the derivative
	@show df_dx_symbolic
	df_dx=build_function(df_dx_symbolic,xₛ,rₛ, expression = Val{false})
	@show df_dx #this is a regular function
	@time x_1=df_dx(0.1,1.5) #the first time may include the compilation time in Julia
	@time x_1=df_dx(0.1,1.5) #runs in a micro-second on my computer
	@show x_1
	nothing
end
...
Write the logistic map $f$ as a sequence of elementary operations, then work out the operations for the infinitesimals and compute the derivative $df/dx$. Then extend the code below to compute a derivative for the logistic map $f$.
"""
...
function df_dx_dual(f::Function, x::Dual)
    y = f(Dual(x.val, 1.0))
    return y.der
end
@show df_dx_dual(sin, Dual(π/3, 1.0))
end

# ╔═╡ ab671ba7-7150-4103-a19a-ca945fede0c6
begin
#plotting and animation for Burgers' equation
function plot_burgers(u, x, t)
    n=length(x)
    plot(x, u, label="t=$(t)", ylims=(0,2))
    xlabel!("x")
    ylabel!("u")
end

function animate_burgers(sol, x, timestep=0.05, fps=30)
	# create an animation
    times = sol.t[1]:timestep:sol.t[end] # times at which to plot the pendulum
    n = length(times)
    anim = @animate for i in 1:n
      	u=sol(times[i])
      	plot_burgers(u,x,times[i])
    end
...
u₀ = 1.0 .+ 0.5.*cos.(k*x)
p=()
burgers=burgers_equation(Δx,ν, similar(u₀), similar(u₀))
tspan=(0.0,5.0)
prob=ODEProblem(burgers,u₀,tspan,p)
sol=solve(prob)
animate_burgers(sol, x)
end
...
In Julia there is no need to implement dual numbers yourself, since the forward mode differentiation is available through the [ForwardDiff package](https://juliadiff.org/ForwardDiff.jl/stable/). In this exercise, we'll use this to find an estimate for the parameter $r$.

First change the program below to compute the sum of the squared differences. Then compute $dJ/dr$ and use gradient descent to find a good estimate of $r$. This results in a very small working parameter estimation method, that is easily extended to larger problems.
"""

# ╔═╡ 8fa1aacd-f464-48a6-9270-44742a9813a6
begin
	# observations : these were generated with an unknown r value, but with correct x0
	# The values correspond to x_1 ... x_10
	z = [0.2700000000000001, 0.5913000000000002, 0.7249929299999999, 0.5981345443500454, 0.7211088336156269, 0.603332651091411, 0.7179670896552621, 0.6074710434816448, 0.7153499244388992, 0.6108732301324811]
	...
	function J(r)
		...
		for i=1:10
			x=f(x,r)
			result=result+x^2 # ADAPT THE CODE
		end
		return result
	end
	...
	dJ_dr = ForwardDiff.derivative(J,r_init)
	@show dJ_dr
...
manifest_format = "2.0"
project_hash = "7317dcde1036a804991699e708269c7dee9e7b9c"
...
git-tree-sha1 = "f30fd5d8e898141f19cc564f00072bb8e2d97d7d"
